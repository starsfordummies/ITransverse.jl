...
@testset "Testing Loschmidt echo optimizers" begin
    ...
    truncp = (;alg="RTMsym")
    pm_params = PMParams(;truncp, cutoffs, maxdims, itermax, eps_converged, normalization="norm")
    psi_svd, ds2 = powermethod_sym(start_mps, mpo, pm_params)

    truncp = (;alg="RTMsym", use_eig=true)
    pm_params = PMParams(;truncp, cutoffs, maxdims, itermax, eps_converged, normalization="overlap")
    psi_eig, ds2 = powermethod_sym(start_mps, mpo, pm_params)

    truncp = (;alg="densitymatrix")
    pm_params = PMParams(;truncp, cutoffs, maxdims, itermax, eps_converged, normalization="norm")
...
@testset "Testing powermethod_lr" begin

    truncp = (;alg="RTMsym")
    pm_params = PMParams(;truncp, cutoffs, maxdims, itermax, eps_converged, normalization="norm")
    left, right, ds2 = ITransverse.powermethod_lr(start_mps, mpo, mpo, pm_params)

    vn_r = vn_entanglement_entropy(right)

    @test  norm( vn_r - vn_ref )/norm(vn_r) < 0.01
