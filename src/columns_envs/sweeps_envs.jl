...
Base.@kwdef struct ConvergenceCriteria
    target_std::Float64    = 1e-6
    target_dchi::Int       = 0       # stop if linkdim stops growing
    min_sweeps::Int        = 3       # always do at least this many
    max_sweeps::Int        = 50
    check_interval::Int    = 3       # how often to compute overlaps (expensive)
    verbose::Bool          = false
end

function rebuild_envs!(left_envs::Environments, right_envs::Environments, 
                       cc::Columns, truncp;
                       crit::ConvergenceCriteria = ConvergenceCriteria(),
                       kwargs...)

    curr_maxL = curr_maxR = 0
    DL = DR = typemax(Int)
    history_std  = Float64[]
    history_chi  = Tuple{Int,Int}[]
    converged    = false

    for jj = 1:crit.max_sweeps

        # First few sweeps: use twocol update to grow bond dim
        if jj <= crit.min_sweeps
            curr_maxL, curr_maxR = sweep_rebuild_envs_rtm_twocol!(left_envs, right_envs, cc, truncp; kwargs...)
        else
            maxL, maxR = sweep_rebuild_envs_rtm!(left_envs, right_envs, cc, truncp; kwargs...)
            DL = maxL - curr_maxL
            DR = maxR - curr_maxR
            curr_maxL, curr_maxR = maxL, maxR
        end

        push!(history_chi, (curr_maxL, curr_maxR))

        # Check overlaps every check_interval sweeps (expensive)
        if jj >= crit.min_sweeps && jj % crit.check_interval == 0
            ov, std = overlap_envs(left_envs, right_envs)
            push!(history_std, std)

            chi_converged = DL == DR == 0
            std_converged = std < crit.target_std
            # Check std is decreasing, not just below threshold
            std_decreasing = length(history_std) < 2 || history_std[end] < history_std[end-1]

            if crit.verbose
                @info "Sweep $jj: χ=($curr_maxL,$curr_maxR) Δχ=($DL,$DR) std=$std"
            end

            if chi_converged && std_converged && std_decreasing
                converged = true
                break
            end
        end
    ...
    converged || @warn "Not converged after $(crit.max_sweeps) sweeps: "*
                       "final χ=($curr_maxL,$curr_maxR), std=$(last(history_std, 1))"

    return converged, history_chi, history_std
end
