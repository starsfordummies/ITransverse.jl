...
function build_expval_tMPS(eH::MPO, init_state::AbstractArray, operator::AbstractArray, time_sites::Vector{<:Index})
   ...
   #close edges
   tMPS[1] *= init_tensor
   tMPS[end] *= init_tensor' # we dag afterwards 
   ...
   # TODO check daggers if we care about symmetries...
   for ii = 1:Nsteps
       tMPS[ii] *= delta(wL, time_sites[ii])  
       tMPS[2*Nsteps+1-ii] *= delta(wL, time_sites[2*Nsteps+1-ii]) 
       tMPS[2*Nsteps+1-ii] = dag(tMPS[2*Nsteps+1-ii]) 
   end

   tMPS[1] *= delta(space_p', rot_links[1] )
   for ii = 2:2*Nsteps-1
       tMPS[ii] *= delta(space_p, rot_links[ii]) * delta(space_p', rot_links[ii-1]) 
   end
